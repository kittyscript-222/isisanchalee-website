...
/**
 * Returns a proxy download URL for a Google Drive file, routed through
 * our own server to avoid Google's cross-origin blocking on uc?export=download URLs.
 */
async function getDownloadUrl(fileId) {
  const site = process.env.SITE_URL || 'https://isisanchalee.com';
  return `${site}/api/proxy-audio?fileId=${fileId}`;
}
...
async function uploadAudioFile(buffer, fileName, mimeType, folderId) {
  ...
  const site = process.env.SITE_URL || 'https://isisanchalee.com';
  const downloadUrl = `${site}/api/proxy-audio?fileId=${fileId}`;
